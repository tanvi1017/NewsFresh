...
class ScienceAdapter (var itemDataList:List<Article>,val newsClickListener:NewsClickInterface) : RecyclerView.Adapter<ScienceAdapter.ItemViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ItemViewHolder, position: Int) {
        ...
        Glide.with(holder.itemView.context).load(model.urlToImage).into(holder.binding.ivImage)
        holder.binding.tvHeadline.text =(model.description)
        val context = holder.itemView.getContext()
        holder.binding.ScienceCardView.setOnClickListener {
